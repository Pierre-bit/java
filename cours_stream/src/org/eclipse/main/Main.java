...
public class Main {
	...
	public static void main(String[] args) {
		...
		List<String> genres = Arrays.asList("Rock","Pop","Jazz");
		
		Long ab = genres.stream().filter(s -> s.startsWith("J")).count();
		...
		List<Integer> together = Stream.of(Arrays.asList(2, 2), Arrays.asList(3, 4)).flatMap(List::stream)
				.map(integer -> integer + 1).distinct().collect(Collectors.toList());
		...
		List<Integer> distinctIntegers = Stream.of(5, 6, 6, 6, 3, 2)
				.distinct()
				.collect(Collectors.toList());
		...
		List<Integer> sortedNumbersASC = Stream.of(5, 3, 1, 3, 6)
				.sorted()
				.collect(Collectors.toList());
		...
		List<Integer> sortedNumbersDESC = Stream.of(5, 3, 1, 3, 6)
		...
		List<Letter> letters = Arrays.asList(
				new Letter("Rue Massena", "Nice"), 
				new Letter("Rue Jean Jaures", "Paris"),
				new Letter("Rue Victor Hugo", "Lyon"), 
				new Letter("Rue Foch", "Marseille")
		);
...
//				.collect(Collectors.toList());
		...
		List<String> noSortedLetters = letters.stream()
				.map(s -> s.getCity())
				.limit(1)
				.collect(Collectors.toList());
